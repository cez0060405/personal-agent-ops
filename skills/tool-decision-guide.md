...
**塌缩防线（2026-08-26 实测补充）**：「起名/想点子/要方案」→ originality；「教会一个人/辅导/备考/教孩子做事」→ tutor——即使措辞生活化（"帮我想想办法让女儿刷牙"）。判 none 前自问：这任务需要**发散多方案**还是需要**教学者视角**？是则必须切身份，别自己硬扛。

### COMPUTER_USE —— 真 GUI 才用（门槛最高，双条件同时满足）
①无 CLI/API/脚本替代路径 ②必须实际操作图形界面/读取屏幕/OS 级窗口操作。
典型：桌面客户端软件（微信电脑版/报表客户端）、Windows 设置面板、读屏、窗口拖放、画图软件手绘过程本身。
